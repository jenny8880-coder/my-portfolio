...
// Switch vibe button
function Frame5({ switchVibeLabel }: { switchVibeLabel: string }) {
  const { theme, switchTheme } = usePersonalization();
  
  const handleClick = () => {
    // Cycle through all themes: vibrant -> calm -> focused -> vibrant
    if (theme === 'vibrant') {
      switchTheme('calm');
    } else if (theme === 'calm') {
      switchTheme('focused');
...
function Frame26({ switchVibeLabel }: { switchVibeLabel: string }) {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-end relative shrink-0 w-full">
      <Frame5 switchVibeLabel={switchVibeLabel} />
      <Frame25 />
    </div>
  );
}
...
export default function VibrantTheme({ data }: VibrantThemeProps) {
  return (
    <div 
          ...
          <div className="relative w-full max-w-[1440px] mx-auto px-[96px] pt-[48px] pb-[96px]">
            ...
            <div className="content-stretch flex gap-[10px] items-center relative shrink-0 w-full mb-[64px]">
              ...
              <div className="basis-0 content-stretch flex flex-col grow items-end min-h-px min-w-px relative shrink-0" style={{ marginRight: '120px', marginTop: '-20px' }}>
                <Frame5 switchVibeLabel={data.navigation.switchVibeButton.label} />
              </div>
            </div>
