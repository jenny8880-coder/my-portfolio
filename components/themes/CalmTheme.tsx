...
function Button2({ label }: { label: string }) {
  const { theme, switchTheme } = usePersonalization();
  
  const handleClick = () => {
    // Cycle through all themes: calm -> focused -> vibrant -> calm
    if (theme === 'calm') {
      switchTheme('focused');
    } else if (theme === 'focused') {
      switchTheme('vibrant');
    } else {
      switchTheme('calm');
    }
...
function Frame7({ navItems, switchVibeLabel }: { navItems: Array<{ label: string }>; switchVibeLabel: string }) {
  return (
    <motion.div 
...
function ProjectCard({ project }: { project: PortfolioData['projects'][0] }) {
  ...
  if (project.id === 'philips') {
    return (
      <Link href="/work/medical" className="block">
        {cardContent}
      </Link>
    );
  }
  ...
  if (project.id === 'kemtai') {
    return (
      <Link href="/work/kemtai" className="block">
        {cardContent}
      </Link>
    );
  }
...
function Frame20({ projects }: { projects: PortfolioData['projects'] }) {
  // Reorder: Left column should have kemtai and EPD, Right column should have adaptive-portfolio and selected-work
  const kemtai = projects.find(p => p.id === 'kemtai');
  const philips = projects.find(p => p.id === 'philips');
  const adaptivePortfolio = projects.find(p => p.id === 'adaptive-portfolio');
  const selectedWork = projects.find(p => p.id === 'selected-work');
  
  const leftColumn = [kemtai, philips].filter(Boolean) as PortfolioData['projects'];
  const rightColumn = [adaptivePortfolio, selectedWork].filter(Boolean) as PortfolioData['projects'];
...
function Frame25({ data }: { data: PortfolioData }) {
  return (
    <div className="relative shrink-0 w-full">
      <div className="flex flex-col items-end size-full">
        <div className="content-stretch flex flex-col gap-[64px] items-end pb-[128px] pt-[44px] px-[36px] relative w-full min-h-screen">
          <Frame7 navItems={data.navigation.items} switchVibeLabel={data.navigation.switchVibeButton.label} />
...
export default function CalmTheme({ data }: CalmThemeProps) {
  return (
